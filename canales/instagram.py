"""
Instagram channel: Meta Graph API webhook handler.

Handles inbound direct messages and sends responses.
"""
...
class InstagramHandler:
        ...
        try:
            ...
            for change in changes:
                ...
                for message in messages:
                    await self._handle_message(client_id, page_id, message)
    ...
    async def _handle_message(
        ...
        """
        Handle inbound message from Instagram.
        ...
        try:
            ...
            logger.info(
                f"Processing Instagram message from {sender_id}",
    ...
    async def send_message(
        ...
        """
        Send message via Instagram Direct Messages API.
        ...
        """
        try:
            credentials = await self._get_client_credentials(client_id, "instagram")

            if not credentials:
                logger.error(f"No Instagram credentials for client {client_id}")
                return False
            ...
            if response.status_code in (200, 201):
                logger.info(
                    f"Instagram message sent to {recipient_id}",
                    extra={"client_id": client_id},
                )
                return True
            else:
                logger.error(
                    f"Failed to send Instagram message: {response.status_code} {response.text}"
                )
                return False
