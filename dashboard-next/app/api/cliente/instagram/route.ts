...
export async function POST(request: Request) {
  try {
    ...
    // Verify credentials and fetch Instagram business account
    const metaRes = await fetch(
      `https://graph.facebook.com/v21.0/${page_id}?fields=name,instagram_business_account&access_token=${access_token}`
    )
    ...
    if (!metaRes.ok) {
      ...
      if (metaRes.status === 401 || metaRes.status === 403) {
        return NextResponse.json(
          { success: false, error: '❌ Token inválido. Verifica que copiaste el token completo y que tiene permisos de instagram_basic e instagram_manage_messages' },
          { status: 422 }
        )
      }
      return NextResponse.json(
    ...
    if (!instagramAccountId) {
      return NextResponse.json(
        { success: false, error: '❌ Esta página de Facebook no tiene una cuenta de Instagram Business conectada. Ve a Configuración de la página → Instagram para vincularla.' },
        { status: 422 }
      )
    }
    ...
    return NextResponse.json({ success: true, instagram_account_id: instagramAccountId })
  } catch (error) {
