...
class AgentEngine:
    ...
    def __init__(self, client_config: dict[str, Any], supabase_client: Any = None, supabase_service_client: Any = None):
        ...
        _APPOINTMENT_RULE = (
            "\n\n🚨 REGLAS CRÍTICAS DE CITAS (MÁXIMA PRIORIDAD — SIN EXCEPCIONES):\n"
            "\n"
            "PARA CREAR UNA CITA:\n"
            "1. PRIMERO llama 'consultar_disponibilidad' con la fecha solicitada.\n"
            "2. Muestra los horarios disponibles al usuario y pídele que elija uno.\n"
            "3. Recolecta: nombre completo, correo electrónico, teléfono, servicio.\n"
            "4. LUEGO llama 'crear_cita' con todos los datos.\n"
            "5. SOLO después de recibir respuesta exitosa de 'crear_cita' confirma al usuario.\n"
            "6. NUNCA confirmes una cita sin haber llamado 'crear_cita' primero.\n"
            "7. NUNCA digas 'te llegará una invitación' sin haber llamado 'crear_cita'.\n"
            "8. NUNCA inventes que agendaste algo — si no llamaste la tool, no está agendado.\n"
            "\n"
        ...
        _COBROS_RULE = (
            ...
            "\n"
            "⚠️ EXCEPCIÓN CRÍTICA: Si el último mensaje del agente fue un recordatorio de cita\n"
            "(contiene 'tienes una cita', 'confirmas tu asistencia', 'tienes tu cita'),\n"
            "NO invocues enviar_datos_bancarios. El usuario está confirmando asistencia,\n"
            "NO iniciando un pago. Responde confirmando su asistencia a la cita.\n"
            "\n"
            "DEBES hacer EXACTAMENTE ESTO cuando SÍ sea sobre pagos (NO hay excepciones):\n"
        ...
        _svc = supabase_service_client or supabase_client
        self.alertas = AlertasModule(_svc) if _svc else None
